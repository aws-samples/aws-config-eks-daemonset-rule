...
def daemonsets_resp(eks_client, cluster):
    cluster_response = eks_client.describe_cluster(name=cluster)
    cert_reqs = ssl.CERT_NONE
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs=cert_reqs)
    endpoint = cluster_response.get('cluster', {}).get('endpoint')
    daemonsets_url = f'{endpoint}/apis/apps/v1/namespaces/kube-system/daemonsets/aws-node'
    token = get_bearer_token(cluster, 'us-west-2')
    response = http.request("GET", daemonsets_url, headers={'Authorization': f'Bearer {token}'}).data
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    for page in cluster_paginator.paginate():
        for cluster in page.get('clusters', []):
            ...
            api_response = daemonsets_resp(eks_client, cluster)
            spec = json.loads(api_response.get('metadata', {}).get('annotations', {}).get('kubectl.kubernetes.io/last-applied-configuration', {}))
            env_variables = spec.get('spec', {}).get('template', {}).get('spec', {}).get('containers', [])[0].get('env', [])
            print('env_variables: ', env_variables)

            for variable in env_variables:
                if (variable['name'] == 'AWS_VPC_K8S_CNI_CUSTOM_NETWORK_CFG') and (variable['value'] == 'true'):
                ...
                elif (variable['name'] == 'AWS_VPC_K8S_CNI_CUSTOM_NETWORK_CFG') and (variable['value'] == 'false'):
                    evaluations.append(build_evaluation(
                        cluster,
                        'NON_COMPLIANT',
                        event,
